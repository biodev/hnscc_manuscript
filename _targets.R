...
list(
    ...
    ## derived file from https://doi.org/10.1016/j.cell.2018.03.035 Table S3
    
    tar_target(
        pancan_pathway_file,
        "data/pancan_pathways_v1.txt",
        format = "file"
    ),
    ...
    tar_target(
        pancan_pathways,
        readr::read_delim(pancan_pathway_file)
    ),
    ...
    tar_target(
        tcga_driver_muts,
        readxl::read_excel("external/nature14129-s2/4.1.xlsx", range="B4:B14", 
    ...
    tar_target(
        ...
        make.mut.oncoprint(mut_data, clin=clin_data, 
    ...
    tar_target(
        gdsc_cna_mat,
        make.gdsc.cna.mat("external/cnv_gistic_20191101.csv", 
                          hnsc_cls$model_id,
    ...
    tar_target(
        gs_tbl_out,
        {readr::write_delim(gene_scores, file="outputs/hnscc_inh_gene_scores.txt", delim="\t", col_names=T); "outputs/hnscc_inh_gene_scores.txt"},
        format="file"
    ),
    ...
    tar_target(
        net_prop_alt_tbl,
        combine.gs.mut.cna.exprs.rppa(net_prop_plus_gs, mut_data, cna_data, 
    ...
    tar_target(
        ...
        plot.all.response.cards(net_prop_alt_tbl, 
